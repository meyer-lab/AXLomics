...
def panel_a_axl_abundance(axl_wb: pd.Series, save_dir: str) -> plt.Figure:
    ...
    ax.bar(
        axl_wb.index, axl_wb.values, color=bar_colors, edgecolor="black", linewidth=0.5
    )
    ...
    plt.tight_layout()
    fig.savefig(
        os.path.join(save_dir, "Panel_A_AXL_abundance.pdf"),
        dpi=300,
        bbox_inches="tight",
    )
    return fig
...
def panel_b_cluster_corr(corrs: list, pvals: list, save_dir: str) -> plt.Figure:
    ...
    ax.axhline(
        0.6, color="gray", linestyle="--", linewidth=0.9, label="|r| = 0.6 threshold"
    )
    ...
    plt.tight_layout()
    fig.savefig(
        os.path.join(save_dir, "Panel_B_cluster_AXL_corr.pdf"),
        dpi=300,
        bbox_inches="tight",
    )
    return fig
...
def panel_c_scatter(
    axl_vec: np.ndarray, centers: np.ndarray, ms_order: list, save_dir: str
) -> plt.Figure:
    ...
    fig.suptitle(
        "Panel C — AXL Abundance vs. Cluster Centers (KO excluded)", y=1.03, fontsize=12
    )
    plt.tight_layout()
    fig.savefig(
        os.path.join(save_dir, "Panel_C_scatterplots.pdf"), dpi=300, bbox_inches="tight"
    )
    return fig


def panel_d_center_comparison(
    centers: np.ndarray,
    centers_resid_perm: np.ndarray,
    pearson_r: list,
    ms_order: list,
    ari: float,
    save_dir: str,
    ...
    plt.tight_layout()
    fig.savefig(
        os.path.join(save_dir, "Panel_D_center_comparison.pdf"),
        dpi=300,
        bbox_inches="tight",
    )
    return fig
...
def panel_e_q2(q2_df: pd.DataFrame, save_dir: str) -> plt.Figure:
    ...
    plt.tight_layout()
    fig.savefig(
        os.path.join(save_dir, "Panel_E_Q2_comparison.pdf"),
        dpi=300,
        bbox_inches="tight",
    )
    return fig
