...
MS_ORDER = ["PAR", "KO", "KD", "WT", "Y634F", "Y643F", "Y698F", "Y726F", "Y750F", "Y821F"]
...
def fit_ddmc(d: pd.DataFrame, info_df: pd.DataFrame):
    ...
    """
    ddmc = DDMC(
        info_df, n_components=5, SeqWeight=2,
        distance_method="PAM250", random_state=5
    ).fit(d)
...
def compare_plsr_models(centers: np.ndarray, axl_vec: np.ndarray, Y: np.ndarray,
                        phenotype_names: list) -> pd.DataFrame:
