...
def preprocess_data(X):
    ...
    xR_nat = xR_nat.drop(
        ["SORBS2", "PKP1", "IGSF10", "PANX1", "PLCXD3", "MYLK2"], axis=0
    )
...
def TumorVsNAT_target(target, xR_nat, xR_tumor):
    ...
    out["Expression"] = list(np.squeeze(xR_nat.loc[target].values)) + list(
        np.squeeze(xR_tumor.loc[target].values)
    )
    return out
...
def find_axl_levels(prot):
    ...
    prot_axl["Levels"] = pd.qcut(
        prot.loc["AXL"], 3, labels=["Low", "Medium", "High"]
    ).values
    return prot_axl
...
def volcano(
    ...
    prot_fc["Prot_Pos"] = [
        pn + "-" + pos
        for pn, pos in list(zip(list(prot_fc["Protein"]), list(prot_fc["Position"])))
    ]
...
def make_AXL_categorical_data(X, prot, phospho=False, by_samples=False, by_thres=False):
    ...
    if by_samples:
        axlHL.columns = [
            l + "_" + c
            for c, l in zip(
                list(axlHL.columns.values), list(hi_and_low["Levels"].values)
            )
        ]
    ...
    if phospho:
        axlHL = pd.melt(
            frame=axlHL,
            id_vars=["Gene", "Position", "Sequence"],
            value_vars=axlHL.columns[2:],
            var_name="AXL",
            value_name="p-site signal",
...
def plot_psites_byAXLlevels(rows, cols, data, protein, figsize=(15, 12)):
    ...
    for ii, p_site in enumerate(list(set(data.loc[protein].index))):
        ...
        sns.violinplot(
            data=dd, x="AXL", y="p-site signal", color="white", ax=ax
        ).set_ylabel(p_site + " log(signal)")
...
def CPTAC_GSEA_YAP_by_AXLprot(rnaR_tumor, protR_tumor):
    rnaHL = make_AXL_categorical_data(
        rnaR_tumor, protR_tumor, phospho=False, by_samples=True
    )
...
def plot_YAPlevels_byStage(phosR_tumor, pmut, ax):
    ...
    phos_cf.insert(
        0,
        "p-site",
        [p + " " + z for p, z in zip(list(phos_cf["Gene"]), list(phos_cf["Position"]))],
    )
