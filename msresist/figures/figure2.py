...
from .common import (
    subplotLabel,
    getSetup,
    import_phenotype_data,
    formatPhenotypesForModeling,
)
...
def makeFigure():
    ...
    # Plot DDMC centers
    X.columns = X.columns = list(X.iloc[:, :7].columns) + [
        "PAR",
        "KO",
        "KD",
        "WT",
...
def plotCenters_together(ddmc, X, ax, drop=None):
    ...
    m = pd.melt(
        centers,
        id_vars=["Cluster"],
        value_vars=list(centers.columns),
        value_name="p-signal",
        var_name="Lines",
    )
...
def ComputeCenters(X, d, i, ddmc):
    ...
    ddmc_data = DDMC(
        i,
        n_components=ddmc.n_components,
        SeqWeight=0,
    ...
    ddmc_seq = DDMC(
        i,
        n_components=ddmc.n_components,
        SeqWeight=ddmc.SeqWeight + 150,
        distance_method=ddmc.distance_method,
        random_state=ddmc.random_state,
    ).fit(d)
    ddmc_seq_c = ddmc_seq.transform()

    return [d, c_kmeans, c_gmm, ddmc_seq_c], [
        "Unclustered",
        "k-means",
        "GMM",
        "DDMC seq",
...
def create_protein_clustermap(data, protein_classes, row_colors=True):
    ...
    data = data.sort_values(
        by=["Cluster", "Protein", "PAR"], ascending=[True, True, False]
    )
    ...
    if row_colors:
        ...
        row_colors_df["Class"] = [
            lut_class.get(gene_to_class.get(gene, "Unknown"), "#CCCCCC")
            for gene in genes
        ]
    else:
        row_colors_df = None
    ...
    g = sns.clustermap(
        ...
        if row_colors
        else None,  # Use DataFrame for row colors
    ...
    if row_colors:
        ...
        handles_cluster = [
            plt.Rectangle((0, 0), 1, 1, color=lut_cluster[i])
            for i in range(1, n_clusters + 1)
        ]

        # Create legend for protein classes
        handles_class = [
            plt.Rectangle((0, 0), 1, 1, color=lut_class[c]) for c in class_categories
        ]
...
protein_classes = {
    "Kinases": ["CDK1", "CDK2", "MAPK1", "MAPK3", "MAPK10", "DYRK3", "TNK2", "PRKCD"],
    "Phosphatases": ["PTPN11", "INPPL1"],
    ...
    "Receptors (including RTKs)": ["EPHA2", "EPHA5", "EPHB3", "EPHB4", "GPRC5B"],
    ...
    "Signaling Regulators / GTPase-related": ["SOS1"],
    "Chaperones / Co-chaperones": ["TCP1", "BAG3"],
    "RNA Splicing / Processing": ["SRRM2"],
    "Oxidoreductases / Enzymes": ["ALDH1A3"],
    "Other / Unknown / Multifunctional": ["TOM1", "SIRPA", "RPS10", "ABI2"],
}
