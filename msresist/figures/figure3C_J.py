...
def makeFigure():
    ...
    cc_egfr = cc_egfr[
        (
            cc_egfr.obs["sampling site (standardized)"]
    ...
    sc.pl.violin(
        cc_egfr, ["AXL score"], rotation=90, groupby="sampling site (standardized)"
    )
    ...
    sc.pl.violin(
        ...
        ["AXL score"],
        rotation=90,
        groupby="sampling site (standardized)",
        order=["Site of Metastasis", "Primary Tumor Site"],
    )
    ...
    sc.pl.violin(
        ...
        groupby="Driver gene",
        order=["EGFR", "BRAF", "ALK"],
    )  # Very few ells in KRAS and ROS1
    sc.pl.violin(
        cc_egfr,
        ["AXL signature"],
        rotation=45,
        groupby="Driver mutation",
        order=["EGFR L858R", "Del19"],
    )
    sc.pl.violin(
        cc_egfr, ["AXL signature"], rotation=45, groupby="Response to treatment"
    )
    sc.pl.violin(
        cc_egfr,
        ["AXL signature"],
...
def preprocess_maynard():
    ...
    adata = adata[adata.obs.perc_mito < 0.3, :]
...
def annotate_maynard(adata):
    ...
    adata.obs.insert(
        ...
        "inferCNV_annotation",
        [
            IDtoCNV[c_id] if c_id in list(IDtoCNV.keys()) else "nonepithelial"
            for c_id in list(adata.obs.index)
        ],
    )

    # Tumor vs NAT
    TvsNAT = adata[
        (adata.obs["Author's cell type"] == "Epithelial")
        & (adata.obs["inferCNV_annotation"] != "NA")
    ]
    cc = adata[
        (adata.obs["Author's cell type"] == "Epithelial")
        & (adata.obs["inferCNV_annotation"] == "cancer cell")
    ]
    adata.obs["Driver mutation"] = adata.obs["Driver mutation"].replace(
        "del19", "Exon 19del"
    )
...
def plot_count_depth(adata, min_count):
    ...
    count_hist = sns.distplot(
        adata.obs["total_counts"], kde=False, bins=log_bins, vertical=True
    )
