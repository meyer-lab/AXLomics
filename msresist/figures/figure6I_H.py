...
from .figure3C_J import preprocess_maynard, annotate_maynard
...
def makeFigure():
    ...
    adata[adata.obs["Driver gene"] == "EGFR"]
    cc[(cc.obs["Driver gene"] == "EGFR")]
    TvsNAT[TvsNAT.obs["Driver gene"] == "EGFR"]
    ...
    C2_3 = X[(X["Cluster"] == 2) | (X["Cluster"] == 3)][
...
def get_YAPsignature_genes():
    cordenonsi_genes = gp.get_library(
        name="MSigDB_Oncogenic_Signatures", organism="Human"
    )["CORDENONSI YAP CONSERVED SIGNATURE"]
    wp4534_genes = gp.get_library(name="WikiPathways_2024_Human")[
        "Mechanoreg And Pathology Of YAP TAZ Hippo And Non Hippo Mechs WP4534"
    ]
    yap_up_genes = gp.get_library(name="MSigDB_Oncogenic_Signatures", organism="Human")[
        "YAP1 UP"
    ]
    return cordenonsi_genes, wp4534_genes, yap_up_genes


def violin_plot_YAPsignatures(
    cc_egfr, C2_3, savefig=False, print_mann_whitney_test=False
):
    """
    ...
    df = sc.get.obs_df(
        cc_egfr,
        ["Treatment timepoint with TKI", "WP4534 YAP", "Cordenonsi YAP", "YAP1 UP"],
    )

    # Melt for easier plotting
    df_melt = df.melt(
        id_vars=["Treatment timepoint with TKI"],
    ...
    sns.violinplot(
        df_melt, x="Signature", y="Score", hue="Treatment timepoint with TKI", ax=ax
    )
...
def print_mann_whitney_results(df):
    ...
    # Cordenonsi YAP
    _, pval3 = mannwhitneyu(
        cordenonsi_scores[PD_mask], cordenonsi_scores[RD_mask], alternative="two-sided"
    )
    _, pval4 = mannwhitneyu(
        cordenonsi_scores[PD_mask], cordenonsi_scores[TN_mask], alternative="two-sided"
    )
    print(f"Cordenonsi YAP: PD vs RD p={pval3:.3e}, PD vs TKI naive (TN) p={pval4:.3e}")

    # WP4534 YAP
    _, pval5 = mannwhitneyu(
        wp4534_scores[PD_mask], wp4534_scores[RD_mask], alternative="two-sided"
    )
    _, pval6 = mannwhitneyu(
        wp4534_scores[PD_mask], wp4534_scores[TN_mask], alternative="two-sided"
    )
    print(f"WP4534 YAP: PD vs RD p={pval5:.3e}, PD vs TKI naive (TN) p={pval6:.3e}")

    # YAP1 UP
    _, pval7 = mannwhitneyu(
        yap1_up_scores[PD_mask], yap1_up_scores[RD_mask], alternative="two-sided"
    )
    _, pval8 = mannwhitneyu(
        yap1_up_scores[PD_mask], yap1_up_scores[TN_mask], alternative="two-sided"
    )
...
# ------------------------------
def permutation_corr_pval(
    x, y, adata, id_col="Subject ID", n_permutations=10000, random_state=None
):
    """
...
# ------------------------------
def plot_perm_result(
    adata, x_name, y_name, n_permutations=10000, random_state=7, additional_y_names=None
):
    """
    ...
    x = adata.obs[x_name]
    y = adata.obs[y_name]
    r, p, null_r, xpb, ypb = permutation_corr_pval(
        x, y, adata, n_permutations=n_permutations, random_state=random_state
    )
    ...
    ax2.axvline(
        r, color=palette[0], linestyle="--", linewidth=2, label=f"{y_name}: r={r:.2f}"
    )

    # Add vertical lines for additional variables
    for i, (add_y_name, result) in enumerate(
        zip(additional_y_names, additional_results), 1
    ):
...
def plot_scRNAseq_gene_set_heatmap(
    ...
    if agg == "gmean":
        pb = plot_df.groupby(id_col, observed=True).agg(
            lambda x: (
                gmean(np.asarray(x, dtype=float) + 1e-8)
    ...
    if sort_columns_by:
        ...
        for feat in sort_columns_by:
            if feat in feat_values:
                ...
                if feat in adata.obs.columns:
                    s = (
                        ...
                        .agg(
                            lambda x: (
                                x.mode().iloc[0] if not x.mode().empty else x.iloc[0]
                            )
                        )
                        .reindex(patient_index)
                    )
            ...
            if sort_orders and feat in sort_orders:
                cat = pd.Categorical(
                    s, categories=list(sort_orders[feat]), ordered=True
                )
                sort_df[feat] = cat
            else:
    ...
    if rows == "genes":
        ...
        pb_z = pd.DataFrame(
            zscore(pb, axis=0, nan_policy="omit"), index=pb.index, columns=pb.columns
        )
        ...
        set_palette = (
            sns.color_palette("Set1", len(unique_sets))
            if unique_sets
            else sns.color_palette("Set1", 1)
        )
        ...
        row_colors_df = pd.DataFrame(
            {
                "Gene Set": [
                    set2color.get(
                        gene_to_set.get(g, unique_sets[0] if unique_sets else ""),
                        (0.5, 0.5, 0.5),
                    )
        ...
        g.cax.set_position([0.97, 0.2, 0.02, 0.3])

    elif rows == "signature_means":
        # map each gene -> set, and build per-signature mean per patient
        sig_to_genes = {
            s: [
                map_upper[g.strip().upper()]
                for g in genes
                if map_upper.get(g.strip().upper(), None) in pb.columns
            ]
        ...
        row_colors_df = pd.DataFrame(
            {"Signature": [sig2color[s] for s in sigs]}, index=sigs
        )
        col_colors_sorted = col_colors.loc[col_order]
    ...
    if features:
        ...
        if all_handles_titles:
            _, axes = plt.subplots(
                1, len(all_handles_titles), figsize=(3.2 * len(all_handles_titles), 2)
            )
