...
def plotViolin_HuTyr_vs_AXLspikedIn():
    ...
    plt.text((x1 + x2) * 0.5, y + 0.2, f"p = {p_value:.2e}", ha="center", fontsize=10)
...
def label_AXLspecificity_peptides_by_DDMC():
    ...
    cluster_labels = [
        X_ms.set_index(["Gene", "Position"]).loc[g, p]["Cluster"]
        for g, p in list(zip(list(X["Gene"]), list(X["Position"])))
    ]
...
def plot_AXL_Screen_Results():
    ...
    X["highlight_PTK2_Y861"] = [
        (gene == "PTK2" and "Y861-p" in pos)
        for gene, pos in zip(X["Gene"], X["Position"])
    ]
    ...
    ax.legend(
        handles,
        ["Other peptides", "PTK2 Y861-p", "Other", "FAK1"],
        title="",
        loc="best",
    )
