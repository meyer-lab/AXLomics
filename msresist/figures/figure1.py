...
def makeFigure():
    ...
    for i, line in enumerate(lines):
        cv_c = IndividualTimeCourses(
            cv,
            96,
            lines,
            tr1,
            tr2,
            "fold-change confluency",
            TimePointFC=24,
            TreatmentFC=False,
            plot=line,
            ax_=ax[i],
        ...
        cv_c["Lines"] = line
        cv_f.append(cv_c)
        red_c = IndividualTimeCourses(
            red,
            96,
            lines,
            tr1,
            tr2,
            "fold-change apoptosis (YOYO+)",
            TimePointFC=24,
            plot=line,
            ax_=ax[i + 10],
            ylim=[0, 13],
        ...
        red_c["Lines"] = line
        red_f.append(red_c)
        sw_c = IndividualTimeCourses(
            sw, 24, lines, tr1, tr2, "RWD %", plot=line, ax_=ax[i + 20], out=True
        )
        sw_c["Lines"] = line
        sw_f.append(sw_c)
...
def make_pval_table_AXLphenotypes(cv_f, red_f, sw_f, ci_f):
    ...
    ciF = pd.concat(ci_f).rename(
        columns={"Condition": "Treatments", "K Estimate": "Data"}
    )
