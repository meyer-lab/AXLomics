...
from ..pre_processing import filter_NaNpeptides, preprocessing
...
def makeFigure():
    ...
    X = Introduce_Correct_DDMC_labels(X, ddmc)
    c123 = X[(X["Cluster"] == 1) | (X["Cluster"] == 2) | (X["Cluster"] == 3)][
        ["Cluster", "Gene", "Position", "PC9 A", "KO A"]
    ]
    ...
    # Import CPTAC data
    phos = filter_NaNpeptides(
        pd.read_csv("msresist/data/MS/CPTAC/CPTAC-preprocessedMotfis.csv").iloc[:, 1:],
        tmt=2,
    )
    prot = (
        pd.read_csv("msresist/data/MS/CPTAC/CPTAC_LUAD_Protein.csv")
    ...
    rna = pd.read_csv("msresist/data/MS/CPTAC/CPTAC_LUAD_RNAseq.csv").drop_duplicates(
        subset="geneSymbol"
    )
    ...
    sns.heatmap(
        pho_df.set_index(["AXL levels", "EGFR mutational status"]).T,
        cmap="coolwarm",
        robust=True,
        ax=ax[1],
    ).set_title("Phosphorylation")

    # DDMC Clusters CPTAC protein data
    prot_df = Protein_by_AXL_levels(protR_tumor, pmut, c123)
    sns.heatmap(
        prot_df.set_index(["AXL", "EGFR mutational status"]).T,
        cmap="coolwarm",
        robust=True,
        vmin=-1.4,
        vmax=1.4,
    ...
    sns.heatmap(
        rna_df.set_index(["AXL", "EGFR mutational status"]).T,
        cmap="coolwarm",
        robust=True,
        vmin=-1.4,
        vmax=1.4,
...
def RNA_by_AXL_levels(rnaR_tumor, protR_tumor, pmut, c123):
    ...
    rna_long.insert(
        3, "EGFR mutational status", [id_to_egfr[s] for s in rna_long["Patient ID"]]
    )
    rna_long["EGFR mutational status"] = rna_long["EGFR mutational status"].replace(
        0, "EGFR WT"
    )
    rna_long["EGFR mutational status"] = rna_long["EGFR mutational status"].replace(
        1, "EGFRm"
    ...
    rna_c1 = rna_long.loc[
        list(ovprna_df[ovprna_df["Cluster"] == 1]["Gene"])
    ].reset_index()
    rna_c1["Cluster"] = 1

    rna_c2 = rna_long.loc[
        list(ovprna_df[ovprna_df["Cluster"] == 2]["Gene"])
    ].reset_index()
    rna_c2["Cluster"] = 2

    rna_c3 = rna_long.loc[
        list(ovprna_df[ovprna_df["Cluster"] == 3]["Gene"])
    ].reset_index()
    rna_c3["Cluster"] = 3
    ...
    rna_df = pd.DataFrame(
        rna_anno.groupby(["AXL", "EGFR mutational status", "Cluster"])[
            "Log(protein expression)"
        ].median()
    ).reset_index()
    rna_df = rna_df.pivot(
        index=["AXL", "EGFR mutational status"],
        columns="Cluster",
        values="Log(protein expression)",
    ).reset_index()
...
def Protein_by_AXL_levels(protR_tumor, pmut, c123):
    ...
    p_long.insert(
        3, "EGFR mutational status", [id_to_egfr[s] for s in p_long["Patient ID"]]
    )
    p_long["EGFR mutational status"] = p_long["EGFR mutational status"].replace(
        0, "EGFR WT"
    )
    p_long["EGFR mutational status"] = p_long["EGFR mutational status"].replace(
        1, "EGFRm"
    ...
    prot_c1 = p_long.loc[
        list(ovprot_df[ovprot_df["Cluster"] == 1]["Gene"])
    ].reset_index()
    prot_c1["Cluster"] = 1

    prot_c2 = p_long.loc[
        list(ovprot_df[ovprot_df["Cluster"] == 2]["Gene"])
    ].reset_index()
    prot_c2["Cluster"] = 2

    prot_c3 = p_long.loc[
        list(ovprot_df[ovprot_df["Cluster"] == 3]["Gene"])
    ].reset_index()
    prot_c3["Cluster"] = 3
    ...
    prot_df = pd.DataFrame(
        prot_anno.groupby(["AXL", "EGFR mutational status", "Cluster"])[
            "Log(protein expression)"
        ].median()
    ).reset_index()
    prot_df = prot_df.pivot(
        index=["AXL", "EGFR mutational status"],
        columns="Cluster",
        values="Log(protein expression)",
    ).reset_index()
...
def Phospho_by_AXL_levels(protR_tumor, phosR_tumor, pmut, c123):
    ...
    id_to_egfr = dict(zip(list(pmut["Sample.ID"]), list(pmut["EGFR.mutation.status"])))
    phos_long.insert(
        3, "EGFR mutational status", [id_to_egfr[s] for s in phos_long["Patient ID"]]
    )
    ...
    phos_long.insert(
        ...
        "AXL levels",
        [
            id_to_axl[s] if s in id_to_axl.keys() else "High"
            for s in phos_long["Patient ID"]
        ],
    )
    ...
    cdk1 = phos_long[
        (phos_long["Gene"] == "CDK1") & (phos_long["Position"] == "Y15-p;T14-p")
    ]
    ...
    pho_c1 = df.loc[
        list(
            zip(
                list(ovp_df[ovp_df["Cluster"] == 1]["Gene"]),
                list(ovp_df[ovp_df["Cluster"] == 1]["Position"]),
            )
        )
    ].reset_index()
    pho_c1["Cluster"] = 1
    ...
    pho_c2 = df.loc[
        list(
            zip(
                list(ovp_df[ovp_df["Cluster"] == 2]["Gene"]),
                list(ovp_df[ovp_df["Cluster"] == 2]["Position"]),
            )
        )
    ].reset_index()
    pho_c2["Cluster"] = 2
    ...
    pho_c3 = df.loc[
        list(
            zip(
                list(ovp_df[ovp_df["Cluster"] == 3]["Gene"]),
                list(ovp_df[ovp_df["Cluster"] == 3]["Position"]),
            )
        )
    ].reset_index()
    pho_c3["Cluster"] = 3
    ...
    pho_df = pd.DataFrame(
        pho_anno.groupby(["AXL levels", "EGFR mutational status", "Cluster"])[
            "norm p-signal"
        ].median()
    ).reset_index()
    pho_df = pho_df.pivot(
        index=["AXL levels", "EGFR mutational status"],
        columns="Cluster",
        values="norm p-signal",
    ).reset_index()
