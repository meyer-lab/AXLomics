...
def formatPhenotypesForModeling(cv, red, sw, c):
    ...
    # Merge and Normalize
    y_ae = pd.concat(
        [v_ae, cd_ae["Apoptosis"], m_ae["Migration"], c_ae["Island"]], axis=1
    )
    y_e = pd.concat([v_e, cd_e["Apoptosis"], m_e["Migration"], c_e["Island"]], axis=1)
    y_ut = pd.concat(
        [v_ut, cd_ut["Apoptosis"], m_ut["Migration"], c_ut["Island"]], axis=1
    )
...
def IndividualTimeCourses(
    ...
    treatments = [[t] * len(lines) for t in t2] * int(
        c.shape[0] * (c.shape[1] - 1) / (len(lines) * len(t1))
    )
    ...
    if plot == "Full":
        _, ax = plt.subplots(
            nrows=2, ncols=5, sharex=True, sharey=True, figsize=figsize
        )
...
def import_phenotype_data(phenotype="Cell Viability", merge=True):
    ...
    if phenotype == "Cell Viability":
        cv1 = pd.read_csv(
            "msresist/data/Phenotypic_data/AXLmutants/CellViability/Phase/BR1_Phase.csv"
        )
        cv2 = pd.read_csv(
            "msresist/data/Phenotypic_data/AXLmutants/CellViability/Phase/BR2_Phase.csv"
        )
        cv3 = pd.read_csv(
            "msresist/data/Phenotypic_data/AXLmutants/CellViability/Phase/BR3_Phase.csv"
        )
        cv4 = pd.read_csv(
            "msresist/data/Phenotypic_data/AXLmutants/CellViability/Phase/BR4_Phase.csv"
        )
        res = FixColumnLabels([cv1, cv2, cv3, cv4])

    elif phenotype == "Cell Death":
        red1 = pd.read_csv(
            "msresist/data/Phenotypic_data/AXLmutants/CellViability/Red/BR1_RedCount.csv"
        )
        red2 = pd.read_csv(
            "msresist/data/Phenotypic_data/AXLmutants/CellViability/Red/BR2_RedCount.csv"
        )
        red3 = pd.read_csv(
            "msresist/data/Phenotypic_data/AXLmutants/CellViability/Red/BR3_RedCount.csv"
        )
        red4 = pd.read_csv(
            "msresist/data/Phenotypic_data/AXLmutants/CellViability/Red/BR4_RedCount.csv"
        )
        ...
        res = (
            DataFrameRipleysK(
                "48hrs", mutants, ["ut", "e", "ae"], 6, np.linspace(1, 14.67, 1)
            )
            .reset_index()
            .set_index("Mutant")
        )
...
def plotVarReplicates(ax, ABC, Set_CorrCoefFilter=False, StdFilter=False):
    ...
    TripsTable = TripsMeanAndStd(
        StdPeptides, merging_indices + ["BioReps"], data_headers
    )
    ...
    ax[0].set_xlabel(
        "Pearson Correlation Coefficients (n=" + str(DupsTable_drop.shape[0]) + ")"
    )
...
def plot_AllSites(ax, x, prot, title, ylim=False, type="Heatmap"):
    ...
    if type == "Heatmap":
        ...
        g = sns.clustermap(
            d.set_index("Positions"), robust=True, cmap="bwr", figsize=(5, 5)
        )
...
def plotMotifs(pssms, axes, titles=False, yaxis=False):
    ...
    for i, ax in enumerate(axes):
        ...
        logo = lm.Logo(
            pssm, vpad=0.1, width=0.8, flip_below=False, center_values=False, ax=ax
        )
...
def DrawArrows(ax, d2):
    ...
    actual_erks = d2[not d2["Shuffled"]]
    arrow_lengths = (
        np.add(
