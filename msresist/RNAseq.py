...
def translate_geneIDs(
    X, labels, toID="entrezgene", export=False, outpath="GSEA_Input.csv"
):
...
def cytoscape_input(ddmc, X):
    ...
    ddmc.store_cluster_members(
        xL,
        "DDMC_PAM250_5Cl_W2_RS5_C",
        ["UniprotAcc", "Gene", "Sequence", "Position", "WTvsKO"],
    )
...
def preprocess_AXL_RNAseq_data():
    ...
    rna.insert(
        1,
        "Treatment",
        [s[-1] if s[-1] == "E" in s else s[-2:] for s in rna["Cell Line"]],
    )
    ...
    rna_f.columns = ["Cell Lines", "Treatment"] + [
        ids[g] if g in ids.keys() else g for g in list(rna_f.columns[2:])
    ]
...
def filter_by_EvEAvar(rna_f, savefig=False, perCut=50):
    ...
    rnaEA = rna_f[
        (rna_f["Treatment"] == "EA")
        | (rna_f["Cell Lines"] == "KO") & ~(rna_f["Treatment"] == "UT")
    ].sort_values(by="Cell Lines")
    ssd = []
    for ii in range(2, rnaE.shape[1]):
        ssd.append(
            np.sum(np.square(rnaEA.iloc[:, ii].values - rnaE.iloc[:, ii].values))
        )
    rna_fEA = rna_f.iloc[:, [True, True] + list(ssd >= np.percentile(ssd, perCut))]
    rna_fEA = rna_fEA[rna_fEA["Treatment"] != "UT"].T
    rna_fEA.columns = [
        c + "-" + t for c, t in list(zip(rna_fEA.iloc[0, :], rna_fEA.iloc[1, :]))
    ]
    rna_fEA = rna_fEA.iloc[2:, :]
    if savefig:
        rna_fEA.to_csv(
            "msresist/WGCNA/WGCNA_input_Filtered_Abundance_and_EvA.txt", sep="\t"
        )
    return rna_fEA
...
def run_standard_gsea_EvsEA(rna_f, gene_sets=gene_sets, outdir=None, out=False):
    ...
    st_gsea_d.insert(
        ...
        [
            c + "_" + t
            for c, t in list(
                zip(list(st_gsea_d["Cell Lines"]), list(st_gsea_d["Treatment"]))
            )
        ],
    )
    st_gsea_d = (
        st_gsea_d.drop(["Cell Lines", "Treatment"], axis=1).set_index("Sample").T
    )
...
def dotplot_std_EvsEA_gsea(rna_f):
    gsea_rnaf_wp = run_standard_gsea_EvsEA(
        rna_f, gene_sets="WikiPathway_2021_Human", outdir=None, out=True
    )
    ...
    wp_selected["-log10(p-value)"] = [
        -np.log10(p + 0.001) for p in wp_selected["NOM p-val"]
    ]
    wp_selected["Hit %"] = [
        np.round((int(s.split("/")[0]) / int(s.split("/")[1])), 2)
        for s in wp_selected["Tag %"]
    ]
    g = sns.PairGrid(
        wp_selected,
        hue="Hit %",
        x_vars="-log10(p-value)",
        y_vars=["Term"],
        height=4,
        aspect=0.75,
    )
...
def PCA_EA(rna_f, wUT=True, plot_pca=False, ax=None):
    ...
    if plot_pca:
        plotPCA_scoresORloadings(
            ax,
            rna_fEA,
            2,
            ["Cell Lines", "Treatment"],
            hue_scores="Cell Lines",
            style_scores="Treatment",
            legendOut=True,
            plot="scores",
        )
    n_components = 4
    ...
    scoresEA.iloc[:, :] = StandardScaler().fit_transform(
        dScor_EA.set_index("Cell Lines")
    )
...
def run_gsea_from_PCA_EA(rna_f, PCA="PC1", plot_pca=False, ax=None, outdir=None):
    if PCA == "PC1":
        ...
        dPC1 = dLoad_EA.reset_index().sort_values(by="PC1", ascending=False)[
            ["Gene", "PC1"]
        ]
